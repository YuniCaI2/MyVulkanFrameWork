        ...
        if(! loader.LoadedMeshes.empty()) {
            for (const auto& mesh : loader.LoadedMeshes) {
                ...
                for (const auto& vertex : mesh.Vertices) {
                    Vertex vertexData;
                    vertexData.pos.x = vertex.Position.X; vertexData.pos.y = vertex.Position.Y; vertexData.pos.z = vertex.Position.Z;
                    vertexData.texCoord.x = vertex.TextureCoordinate.X; vertexData.texCoord.y = vertex.TextureCoordinate.Y;
                    vertexData.color =vertexData.pos;
                    vertexData.normal.x = vertex.Normal.X; vertexData.normal.y = vertex.Normal.Y; vertexData.normal.z = vertex.Normal.Z;
...
void VK::Instances::Model::createModelTextureImage(const VK::Device &device, const VkCommandPool &commandPool) {
    for (auto & mesh : meshes) {
        ...
        stagingBuffer.UnMap();
        //这里先不做mipmap
        mesh.texture.image.createImage(device, resizedImage.cols, resizedImage.rows,1,
